...
import { Droplets, Search, Plus, X, Trash2, Eye, CheckCircle, Clock, Truck, XCircle } from "lucide-react";
import AnimatedNumber from "../../components/AnimatedNumber";
import { SkeletonTable } from "../../components/Skeleton";

const STATUS_OPTIONS = [
  { value: "pending", label: "Pending", color: "bg-gray-100 text-gray-600" },
  { value: "dalam_perjalanan", label: "Dalam Perjalanan", color: "bg-blue-100 text-blue-600" },
...
export default function WaterDistribution() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await waterDistributionService.createWaterDistribution(form);
      resetForm();
      loadItems();
    } catch (err) {
  ...
  const handleStatusChange = async (id, status) => {
    try {
      await waterDistributionService.updateWaterDistributionStatus(id, { status, note: `Status diubah ke ${status}` });
      loadItems();
    } catch {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      alert("Gagal menghapus data distribusi");
    }
  };
  ...
  const filteredItems = items.filter((item) => {
    ...
    return (
      ...
      item.lokasi_distribusi?.toLowerCase().includes(q) ||
      item.status?.toLowerCase().includes(q)
    );
  });

  const countByStatus = (s) => items.filter((i) => i.status === s).length;
  const totalKubikasi = items.reduce((sum, i) => sum + (Number(i.jumlah_kubikasi) || 0), 0);
    ...
    <div className="p-6 lg:p-8 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Distribusi Air Bersih</h1>
              <p className="text-gray-500 text-sm mt-0.5">Kelola pendistribusian air bersih ke daerah terdampak</p>
            </div>
          </div>
        </div>
        {adminUser && (
      ...
      {showForm && adminUser && (
        <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/60 border border-gray-100 p-8 mb-8 animate-slide-in">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-gray-900">Formulir Distribusi Air Bersih</h2>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">
                Nama Distribusi <span className="text-red-500">*</span>
              </label>
              <input name="nama_distribusi" value={form.nama_distribusi} onChange={handleChange} required placeholder="Contoh: Distribusi ke RT 01" className={inputClass} />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Lokasi Distribusi</label>
              <input name="lokasi_distribusi" value={form.lokasi_distribusi} onChange={handleChange} placeholder="Alamat lokasi distribusi" className={inputClass} />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Jumlah Kubikasi (m³)</label>
              <input name="jumlah_kubikasi" type="number" step="0.01" value={form.jumlah_kubikasi} onChange={handleChange} placeholder="0.00" className={inputClass} />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Target Penerima</label>
              <input name="target_penerima" value={form.target_penerima} onChange={handleChange} placeholder="Contoh: 50 KK" className={inputClass} />
            </div>
            <div className="md:col-span-2">
              <label className="block text-sm font-semibold text-gray-700 mb-2">Keterangan</label>
              <textarea name="keterangan" value={form.keterangan} onChange={handleChange} rows={3} placeholder="Keterangan tambahan..." className={inputClass} />
            </div>
            <div className="md:col-span-2 flex gap-3">
              <button type="submit" className="flex-1 btn btn-primary py-3.5">Simpan Distribusi</button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className={`stat-card bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-all group ${showContent ? "show" : ""}`} style={{ transitionDelay: "0s" }}>
          ...
          <p className="text-xs text-gray-500 mt-1">Total distribusi</p>
        </div>
        <div className={`stat-card bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-all group ${showContent ? "show" : ""}`} style={{ transitionDelay: "0.08s" }}>
          ...
          <p className="text-xs text-gray-500 mt-1">Menunggu distribusi</p>
        </div>
        <div className={`stat-card bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-all group ${showContent ? "show" : ""}`} style={{ transitionDelay: "0.16s" }}>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[11px] font-bold uppercase tracking-wider text-gray-400">Dalam Perjalanan</span>
          </div>
          <p className="text-3xl font-extrabold text-gray-900"><AnimatedNumber value={countByStatus("dalam_perjalanan")} /></p>
          <p className="text-xs text-gray-500 mt-1">Sedang dikirim</p>
        </div>
        <div className={`stat-card bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-all group ${showContent ? "show" : ""}`} style={{ transitionDelay: "0.24s" }}>
          ...
          <p className="text-xs text-gray-500 mt-1">{totalKubikasi.toFixed(1)} m³ total</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/60 border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-100">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-bold text-gray-700 uppercase tracking-wider">Daftar Distribusi Air Bersih</h3>
          </div>
          <div className="relative">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
            <input
              type="text"
              placeholder="Cari kelurahan, nama distribusi, atau status..."
              value={searchTerm}
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50/80">
                  <th className="py-4 px-6 text-xs font-bold uppercase tracking-wider text-gray-500">Nama Distribusi</th>
                  ...
                  <th className="py-4 px-6 text-xs font-bold uppercase tracking-wider text-gray-500 text-center">Kubikasi</th>
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-blue-50/40 transition-colors group">
                        <td className="py-4 px-6">
                          ...
                          {item.target_penerima && <p className="text-xs text-gray-500 mt-0.5">Target: {item.target_penerima}</p>}
                        </td>
                        <td className="py-4 px-6">
                        ...
                        <td className="py-4 px-6 text-sm text-gray-600">{item.lokasi_distribusi || "-"}</td>
                        <td className="py-4 px-6 text-center text-sm font-semibold text-gray-900">
                          {item.jumlah_kubikasi ? `${item.jumlah_kubikasi} m³` : "-"}
                        </td>
                      ...
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <div>
                          <p className="text-lg font-semibold text-gray-700">
                            {searchTerm ? "Tidak ada hasil" : "Belum ada data distribusi air"}
                          </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowDetail(null)}>
          <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full mx-4 p-6 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-gray-900">Detail Distribusi Air Bersih</h3>
            ...
            <div className="space-y-3 text-sm">
              <div><span className="font-semibold text-gray-700">Nama Distribusi:</span> {showDetail.nama_distribusi}</div>
              ...
              <div><span className="font-semibold text-gray-700">Lokasi Distribusi:</span> {showDetail.lokasi_distribusi || "-"}</div>
              <div><span className="font-semibold text-gray-700">Jumlah Kubikasi:</span> {showDetail.jumlah_kubikasi ? `${showDetail.jumlah_kubikasi} m³` : "-"}</div>
              <div><span className="font-semibold text-gray-700">Target Penerima:</span> {showDetail.target_penerima || "-"}</div>
