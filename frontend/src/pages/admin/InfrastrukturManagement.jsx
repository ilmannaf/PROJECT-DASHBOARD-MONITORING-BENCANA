...
import { Building, Search, Plus, X, Trash2, Eye, CheckCircle, Clock, HardHat } from "lucide-react";
...
const STATUS_OPTIONS = [
  ...
  { value: "dalam_pengerjaan", label: "Dalam Pengerjaan", color: "bg-orange-100 text-orange-600" },
...
export default function InfrastrukturManagement() {
  ...
  const resetForm = () => {
    setForm({ disaster_record_id: "", kelurahan: "", kecamatan: "", usulan_description: "", aset_milik_opd_lain: "false", opd_nama: "" });
    setShowForm(false);
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await bidang3Service.createInfrastrukturProposal(form);
      resetForm();
      loadItems();
    } catch (err) {
  ...
  const handleStatusChange = async (id, status) => {
    try {
      await bidang3Service.updateInfrastrukturStatus(id, { status, note: `Status diubah ke ${status}` });
      loadItems();
    } catch {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await bidang3Service.deleteInfrastrukturProposal(id);
      loadItems();
    } catch {
  ...
  const handleViewDetail = async (id) => {
    try {
      const detail = await bidang3Service.getInfrastrukturProposalById(id);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-purple-500 to-indigo-600 text-white flex items-center justify-center shadow-lg shadow-purple-500/30">
              <Building className="w-6 h-6" />
            </div>
            <div>
              <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Usulan Infrastruktur</h1>
              <p className="text-gray-500 text-sm mt-0.5">Kelola usulan bantuan infrastruktur dari kelurahan</p>
            </div>
          </div>
        </div>
        {adminUser && (
      ...
      {showForm && adminUser && (
        <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/60 border border-gray-100 p-8 mb-8 animate-slide-in">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-gray-900">Formulir Usulan Infrastruktur</h2>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              ...
              <textarea name="usulan_description" value={form.usulan_description} onChange={handleChange} rows={3} placeholder="Jelaskan kebutuhan infrastruktur..." className={inputClass} />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Aset Milik OPD Lain?</label>
              <select name="aset_milik_opd_lain" value={form.aset_milik_opd_lain} onChange={handleChange} className={inputClass}>
                <option value="false">Tidak</option>
                <option value="true">Ya</option>
              </select>
            </div>
            {form.aset_milik_opd_lain === "true" && (
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Nama OPD</label>
                <input name="opd_nama" value={form.opd_nama} onChange={handleChange} placeholder="Nama OPD yang memiliki aset" className={inputClass} />
              </div>
            )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className={`stat-card bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-all group ${showContent ? "show" : ""}`} style={{ transitionDelay: "0.16s" }}>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[11px] font-bold uppercase tracking-wider text-gray-400">Pengerjaan</span>
          </div>
          <p className="text-3xl font-extrabold text-gray-900"><AnimatedNumber value={countByStatus("dalam_pengerjaan")} /></p>
          <p className="text-xs text-gray-500 mt-1">Dalam pengerjaan</p>
        </div>
      ...
      <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/60 border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-100">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-bold text-gray-700 uppercase tracking-wider">Daftar Usulan Infrastruktur</h3>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50/80">
                  ...
                  <th className="py-4 px-6 text-xs font-bold uppercase tracking-wider text-gray-500">Deskripsi</th>
                  ...
                  filteredItems.map((item) => {
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-purple-50/40 transition-colors group">
                        ...
                        <td className="py-4 px-6 text-sm text-gray-600 max-w-[200px] truncate">{item.usulan_description || "-"}</td>
                      ...
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <div>
                          <p className="text-lg font-semibold text-gray-700">
                            {searchTerm ? "Tidak ada hasil" : "Belum ada usulan infrastruktur"}
                          </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowDetail(null)}>
          <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full mx-4 p-6 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-gray-900">Detail Usulan Infrastruktur</h3>
            ...
            <div className="space-y-3 text-sm">
              ...
              <div><span className="font-semibold text-gray-700">Aset OPD Lain:</span> {showDetail.aset_milik_opd_lain ? "Ya" : "Tidak"}</div>
              {showDetail.aset_milik_opd_lain === 1 && (
                <div><span className="font-semibold text-gray-700">Nama OPD:</span> {showDetail.opd_nama || "-"}</div>
              )}
